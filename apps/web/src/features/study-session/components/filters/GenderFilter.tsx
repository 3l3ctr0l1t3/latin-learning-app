...
import GroupsIcon from '@mui/icons-material/Groups'; // Para común (ambos géneros)
...
const genderConfig: Record<Gender, {
  ...
  common: {
    label: 'Común',
    icon: <GroupsIcon />,
    color: LATIN_COLORS.genders.common || '#9C27B0', // Púrpura para común, con fallback si no está definido
    examples: ['parens', 'infans', 'sacerdos'],
    description: 'Sustantivos que pueden ser masculinos o femeninos'
  }
};
...
const GenderFilter: React.FC<GenderFilterProps> = ({
   ...
   */
  const selectAll = () => {
    onChange(['masculine', 'feminine', 'neuter', 'common']);
  };
    ...
    <Box>
      ...
      <Box sx={{ 
        ...
        <Stack direction="row" spacing={1}>
          <Chip
            ...
            onClick={selectAll}
            disabled={disabled}
            variant={selectedGenders.length === 4 ? "filled" : "outlined"}
            sx={{ cursor: 'pointer' }}
          />
          <Chip
            label="Ninguno"
      ...
      <Box sx={{ mt: 2 }}>
        <Typography variant="caption" color="text.secondary">
          ...
          {selectedGenders.length === 3 && `3 géneros seleccionados`}
          {selectedGenders.length === 4 && 'Todos los géneros seleccionados'}
        </Typography>
      </Box>
    </Box>
  );
